...
type Graph[K comparable, V any] struct {
	...
	AllPairShortestPaths map[types.Tuple2[K, K]]Route[K, V] `json:"AllPairShortestPath"`
}
...
func (g *Graph[K, V]) CalculateAllPairShortestPath() {
	...
	// Create a map
	routeMap := make(map[types.Tuple2[K, K]]Route[K, V],
	...
	for from, connections := range g.Vertices {
		for to := range g.Vertices {
			if connections.Exists(to) {
				...
				routeMap[types.Tuple2[K, K]{from, to}] = Route[K, V]{
					Vertices: []K{from, to},
	...
	for intermediate := range g.Vertices {
		for from := range g.Vertices {
			for to := range g.Vertices {
				var (
					routeFI = routeMap[types.Tuple2[K, K]{from, intermediate}]
					distFI  = routeFI.Distance
					routeIT = routeMap[types.Tuple2[K, K]{intermediate, to}]
					distIT  = routeIT.Distance
					distFT  = routeMap[types.Tuple2[K, K]{from, to}].Distance
				...
				if isValidIntermediate && isDetourBetter {
					...
					routeMap[types.Tuple2[K, K]{from, to}] = routeFI // includes appended route to To
				}
			}
		}
	}
...
func (g *Graph[K, V]) GetShortestRoute(from K, to K) Route[K, V] {
	...
	route := g.AllPairShortestPaths[types.Tuple2[K, K]{from, to}]
	return route
}
